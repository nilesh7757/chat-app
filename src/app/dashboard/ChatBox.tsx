...
export default function ChatBox({
  ...
  const getProperFileName = (fileName: string, fileType: string) => {
    // Check if fileName already has an extension
    const hasExtension = fileName.includes(".") && fileName.split(".").pop()!.length <= 4

    if (hasExtension) {
      return fileName // Already has extension, return as is
    }
    ...
    return fileName // Return original if no extension can be determined
  }
  ...
  const handleFileDownload = async (fileUrl: string | undefined, fileName: string) => {
    ...
    try {
      ...
      // For Cloudinary URLs, add the attachment flag
      ...
      <div className="p-6 border-b border-gray-200 bg-white shadow-sm">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-lg font-semibold text-gray-900 truncate">{contact?.name || targetEmail}</p>
            <p className="text-sm text-gray-500 truncate">{targetEmail}</p>
          </div>
        </div>
      </div>
        ...
        {messages.length === 0 ? (
          ...
          <div className="space-y-4 max-w-full">
            {messages.map((msg, i) => {
              ...
              return (
                <div key={i} className={`relative flex ${isOwnMessage ? "justify-end" : "justify-start"} px-2 group`}>
                  {isOwnMessage && (
